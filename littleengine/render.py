...
def shade(objects, lights, intersection_points, object_indices):
    ...
    for light in lights:
        ...
        for i in range(n):
            print(i)
            shadow_ray_t, shadow_ray_indices, t = trace(objects, intersection_points[i], normalized_light_directions)
            shadow_hit[i] = shadow_ray_t[i]
            o_i[i] = shadow_ray_indices[i]
            truth = (shadow_ray_indices != -1) & (shadow_ray_t * shadow_ray_t < len2[i])
            inShadow[i] = truth[i]
            # print(shadow_ray_t[i], shadow_ray_indices[i], truth[i])
        # print(shadow_hit, o_i, inShadow)
...
def render2(w, h, cam, objects, lights):
    """
    ...
    st = time.time()
    min_t_values, object_indices, tri_indices = trace(objects, cam.position, ray_vectors)
    print(time.time() - st)
    ...
    hit_color_image = np.zeros((h, w, 3), dtype=np.uint8)
...
def render(w, h, cam, objects, lights):
    ...
    st = time.time()
    ray_vectors = camera_rays(w, h, cam)
    _, object_indices = trace(objects, cam.position, ray_vectors)
    print(time.time() - st)
    ...
    for i in range(w):
        for j in range(h):
            if object_indices[j, i] != -1:
                pixel_buffer[i, j] = tuple(objects[object_indices[j, i]].color)
            else:
